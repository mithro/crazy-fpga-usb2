...
| `usb2soft/platforms/netv2.py` | `NeTV2Platform` (variants `a7-35`/`a7-100`, toolchain `Vivado`/`Xray`), all pins, `toolchain_prepare` override adding utilisation/timing report copies |
...
class _Blink(Elaboratable):
    """Requests one LED, the clock, HDMI RX0 lane 0 and the USB pair, so the XDC lists them."""
    def elaborate(self, platform):
        ...
        m.d.comb += led_buf.o.eq(d0.i ^ usb_buf.i)
        return m
...
def test_xdc_contains_expected_pins(tmp_path):
    ...
    assert "create_clock" in xdc and "20.0" in xdc  # 50 MHz default clock constraint
...
class NeTV2Platform(XilinxPlatform):
    ...
    resources = [
        ...
        *[Resource("led", i, PinsN(p, dir="o"), _LVCMOS33)
        ...
        Resource("usb_hax", 0, Subsignal("d", DiffPairs("E19", "D19")), _LVCMOS33),
...
Run: `uv run pytest tests/test_platform_netv2.py -q`
Expected: `6 passed`. If `plan.files["t.xdc"]` lacks `create_clock`, check that `_Blink` did not request `clk50` explicitly (the default `sync` domain machinery adds the constraint only when the domain is created; if needed, request `clk50` with `dir="-"` inside `_Blink` and add `platform.add_clock_constraint(...)` is *not* required — amaranth emits `create_clock` for resources carrying `Clock()`). Adjust the assertion to `"20.0" in xdc or "50" in xdc` only after reading the generated XDC.
...
def test_netv2_bringup_pll():
    sol = solve(kind="pll", speed="-2", fin=50e6, outputs={"usb": 60e6, "idelay_ref": 300e6})
...
def test_default_mmcm_plan():
    sol = solve(kind="mmcm", speed="-2", fin=60e6,
...
def test_phase_is_carried_and_validated():
    ...
    with pytest.raises(ValueError, match="phase"):
        ...
        solve(kind="mmcm", speed="-2", fin=60e6, outputs={"a": 480e6, "b": (120e6, 20.0)})
...
def test_minus_one_3x_plan():
    sol = solve(kind="mmcm", speed="-1", fin=60e6,
                outputs={"rx_io": 360e6, "tx_io": 240e6, "rx_cdr": 120e6, "usb": 60e6}, max_bufg=464e6)
    assert sol.vco == 720e6 and sol.mult == 12


def test_prefers_highest_vco():
    # 50 MHz -> 100 MHz could use VCO 800 (x16) ... 1440 (x28.8 not integer); expect 1400 (x28)? No:
...
def solve(*, kind, speed, fin, outputs, max_bufg=None, prefer="highest_vco"):
    ...
    for divclk in range(lim.divclk[0], lim.divclk[1] + 1):
        ...
        for mult in range(lim.mult[0], lim.mult[1] + 1):
            vco = pfd * mult
            if not lim.vco[0] <= vco <= lim.vco[1]:
                continue
            settings = {}
            ok = True
            ...
            if ok:
                candidates.append(ClockSolution(kind=kind, fin=fin, divclk=divclk, mult=mult,
                                                vco=vco, outputs=settings))
    if not candidates:
        raise ValueError(f"no solution for {kind} {speed} fin={fin/1e6:.3f} MHz outputs={wanted}")
    # Prefer the fewest input dividers (highest PFD frequency), then the highest VCO.
    candidates.sort(key=lambda s: (s.divclk, -s.vco))
    return candidates[0]
```

Note on `test_prefers_highest_vco`: with `divclk=1`, `fin=50e6`, integer `mult` and `100e6 | vco` the candidates are 800, 900, …, 1400 MHz; sorting by `(divclk, -vco)` picks 1400 MHz (mult 28, divide 14). The `test_phase_is_carried_and_validated` negative case must raise before any candidate is accepted — it does, because the phase check runs inside the loop for every candidate; to keep the error deterministic the check is raised the first time it is encountered.
...
class _Top(Elaboratable):
    def elaborate(self, platform):
        m = Module()
        m.submodules.clocks = NeTV2BringupClocks()
        counter = Signal(4)
...
class NeTV2BringupClocks(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        sol = solve(kind="pll", speed=f"-{platform.speed}", fin=50e6, outputs=self.OUTPUTS)
        m.submodules.pll = pll = PLLE2(sol, clkin=clk50_buf.i)

        m.domains.usb = ClockDomain("usb")
        m.domains.idelay_ref = ClockDomain("idelay_ref")
        m.domains.sync = ClockDomain("sync")
        m.d.comb += [
            ClockSignal("usb").eq(pll.clocks["usb"]),
            ClockSignal("idelay_ref").eq(pll.clocks["idelay_ref"]),
            ClockSignal("sync").eq(pll.clocks["usb"]),
...
The platform's `create_missing_domain` would otherwise build `sync` straight from `clk50`; defining `sync` here overrides that, so applets can use `m.d.sync` at 60 MHz. Add `platform.add_clock_constraint(clk50_buf.i, 50e6)` only if the generated XDC lacks `create_clock` (the `Clock(50e6)` on the resource should already produce it).
            ...
            with m.State("STOP"):
                with m.If(baud == 0):
                    with m.If(rx):
                        ...
                        m.d[self.domain] += self.data.eq(shift)
                    with m.Else():
...
Add `from amaranth import Cat` to the import line. Watch the `valid`/`data` timing: `data` is registered on the same edge that `valid` is combinationally asserted, so a consumer sampling `data` when it sees `valid` gets the *previous* value. Fix this in the implementation, not the test: make `valid` registered too (`m.d[self.domain] += self.valid.eq(1)` in STOP with a default `m.d[self.domain] += self.valid.eq(0)` at the top instead of the comb default). Do the same for `error`.
...
def _collect(dut, trigger_twice=False):
    ...
    async def tb(ctx):
        ...
        if trigger_twice:
            ctx.set(dut.rep.trigger, 1)      # ignored while busy
            await ctx.tick()
            ctx.set(dut.rep.trigger, 0)
        for _ in range(60):
...
from amaranth import Elaboratable, Module, Signal, Cat, Array, Mux
    ...
    def elaborate(self, platform):
        ...
        for entry in self._plan:
            if entry[0] == "lit":
                ...
                _, idx, nib = entry
                nibble = latched[idx].word_select(nib, 4) if nib < ceil(len(latched[idx]) / 4) else 0
                byte_at.append(hex_lut[nibble])
        table = Array(byte_at)
...
`word_select` on a signal whose width is not a multiple of 4 needs padding: build `padded = Cat(latched[idx], Const(0, pad))` first. Do that rather than the inline conditional shown.
...
LUNA's `UARTTransmitter` uses `m.d.sync`; the applets alias `sync` to the 60 MHz `usb` domain (Task 4), so `domain` must be `"sync"` for the console. Assert that in `Console.__init__`.
...
def build_applet(name, *, platform, variant, toolchain, build_root="build", do_build=True, args=None):
    ...
    if not do_build:
        products.extract()  # writes the files into build_dir
...
`BuildPlan.extract()` with no arguments writes into the current directory; call it as `products.extract(str(build_dir))`? In amaranth 0.5 `BuildPlan.extract(root="build")` takes the root path and `execute_local` writes there; check the signature with `uv run python - <<'EOF'` style script in `tmp/` and adapt (the test asserts `top.v` exists under `build_dir`).
...
def load_command(profile, remote_bit):
    ...
    xc7 = ""  # the bcm2835 cfg sources xc7 itself; the gpiod cfg needs it added
    if profile.openocd_cfg == "netv2-jtag-gpiod.cfg":
        xc7 = " -f /usr/share/openocd/scripts/fpga/xlnx/xc7.cfg"
    sudo = "sudo " if profile.sudo else ""
    return f'{sudo}openocd -f {cfg}{xc7} -c "init; scan_chain; pld load 0 {remote_bit}; exit"'
...
from amaranth import Elaboratable, Module, Signal, Array, Cat, Const
from amaranth.lib import io
from luna.gateware.interface.uart import UARTTransmitter
...
class DiscoveryCore(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        m.submodules.line = line = Console([b"D ", Hex(self.ident), b" R", Hex(lane_code), b" ",
                                           ...
                                           divisor=self.console_divisor)
        ...  # sequencer: every report_period emit header, then lines for sel = 0..n_lanes-1,
             # waiting for each console's reporter.busy to fall; OR the two tx lines (both idle high)
        return m
```

Finish the sequencer as a small FSM (`WAIT` counting `report_period`, `HEADER` triggering `header`, `LINE` triggering `line` for each `sel`, each advancing when `reporter.busy` drops after the trigger). Two `Console`s each own a `UARTTransmitter`; ORing `tx` is wrong (idle is high) — AND them instead (`self.uart_tx.eq(header.tx & line.tx)`), which is safe because they never transmit simultaneously. Note `Hex(lane_code)` prints two nibbles, so the test expects `R01`, `R02`; and `Hex(flags)` prints `03` / `01`.
...
Run: `uv run pytest tests/test_hdmi_discovery.py -q`
Expected: `5 passed`. The `test_core_reports_lines` timing constants (report_period 2500, 9000 cycles) may need tuning once the sequencer exists: compute one line = 33 bytes × 10 bits × DIV(4) = 1320 cycles; 5 lines (header + 4) ≈ 6600 cycles; the run must exceed `report_period + 6600`. Adjust the run length in the test, not the design.
