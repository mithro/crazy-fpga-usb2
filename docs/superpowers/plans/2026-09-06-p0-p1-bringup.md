...
class DiscoveryCore(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        # Report sequencer. A Console's reporter.busy is combinational from its SEND state, so it
        # is low in the cycle the trigger is applied and high from the next cycle until the last
        # byte is accepted; the *_WAIT states are entered one cycle after the trigger and are
        # therefore safe to leave on ~busy. The UART transmitter drains its last byte after busy
        # falls, which is fine because the next line cannot start faster than the transmitter's
        # stream.ready allows.
        ...
        with m.FSM():
            ...
            with m.State("HEADER_WAIT"):
                with m.If(~header.reporter.busy):
                    m.next = "LINE"
            with m.State("LINE"):
                m.d.comb += line.trigger.eq(1)
                m.next = "LINE_WAIT"
            with m.State("LINE_WAIT"):
                with m.If(~line.reporter.busy):
...
Byte budget for the test: a lane line is 27 bytes (`D `2 + id 6 + ` R`2 + lane 2 + space + peer 6 + space + pl 2 + space + flags 2 + CRLF 2), the header 15; at DIV=4 one byte is 40 cycles, so header + 4 lines ≈ 4,920 cycles and `report_period=2500` plus one report fits comfortably in the 9,000-cycle run. `Hex(lane_code)` prints two nibbles, so the test expects `R01`, `R02`; `Hex(flags)` prints `03` / `01`.
